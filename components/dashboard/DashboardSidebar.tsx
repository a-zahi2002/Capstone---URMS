...
const roleMeta: Record<string, { label: string; color: string; accent: string; gradient: string; dot: string }> = {
    admin:       { label: "Administrator", color: "text-purple-500",  accent: "purple", gradient: " ", dot: "bg-purple-500" },
    lecturer:    { label: "Lecturer",      color: "text-emerald-500", accent: "emerald", gradient: " ", dot: "bg-emerald-500" },
    student:     { label: "Student",       color: "text-blue-500",    accent: "blue",    gradient: " ",    dot: "bg-blue-500" },
    maintenance: { label: "Maintenance",   color: "text-amber-500",   accent: "amber",   gradient: " ", dot: "bg-amber-500" },
};
    ...
    const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
        <div className="flex flex-col h-full">
                    ...
                    return (
                        <Link
                            ...
                            className={`
                                ...
                                ${isActive
                                    ? `bg-card from-${meta.accent}-500/10 to-${meta.accent}-500/5 ${meta.color} font-black`
                        ...
                        >
                            ...
                            {isActive && (
                                <motion.span
                                    ...
                                    className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-card ${meta.gradient}`}
                            ...
                            {item.badge && item.badge > 0 && (!collapsed || isMobile) && (
                                <span className={`ml-auto text-[10px] font-black bg-${meta.accent}-500/15 ${meta.color} px-2 py-0.5 rounded-none`}>
                                    {item.badge}
                                </span>
                            )}
                            {/* Tooltip for collapsed state */}
                            {collapsed && !isMobile && (
                                <div className="absolute left-full ml-3 px-2.5 py-1.5 bg-slate-900 dark:bg-slate-700 text-white text-xs font-bold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none whitespace-nowrap z-50 border-2 border-foreground shadow-[4px_4px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,0.2)]">
                                    {item.label}
            ...
            <div className="shrink-0 border-t border-slate-200/80 dark:border-white/[0.06] p-3 space-y-2">
                {/* User card */}
                <div className={`flex items-center gap-3 rounded-none p-2.5 bg-slate-50 dark:bg-white/[0.03] ${collapsed && !isMobile ? "justify-center p-2" : ""}`}>
                    <div className={`w-8 h-8 rounded-lg bg-card ${meta.gradient} flex items-center justify-center text-white text-xs font-black shadow shrink-0`}>
                        {initials}
                    </div>
                    {(!collapsed || isMobile) && (
                ...
                <button
                    onClick={signOut}
                    className={`w-full flex items-center gap-3 rounded-none px-3 py-2.5 text-sm font-semibold text-red-500/80 hover:text-red-500 hover:bg-red-500/5 transition-all ${collapsed && !isMobile ? "justify-center px-0" : ""}`}
