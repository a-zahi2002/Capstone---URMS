    ...
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8">
            ...
            {pending.length > 0 && (
                <motion.div
                    variants={fadeInUp}
                    className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-none"
                >
            ...
            <motion.div variants={fadeInUp} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-card   dark: dark: rounded-none p-8 text-white border-2 border-foreground shadow-[4px_4px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,0.2)] shadow-blue-900/20 relative overflow-hidden border border-blue-500/30">
                    ...
                    {nextBooking ? (
                        <div className="relative z-10">
                            ...
                            <Link
                                href="/bookings"
                                className="mt-6 inline-flex items-center gap-2 bg-white/20 hover:bg-white/30  text-white px-5 py-2.5 rounded-none font-bold text-sm transition-colors"
                            >
                ...
                <div className="bg-card border border-slate-200 dark:border-border rounded-none p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Link
                            href="/resources"
                            className="flex flex-col items-center justify-center p-5 bg-slate-50 dark:bg-white/[0.03] rounded-none hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-all border border-slate-200 dark:border-border group"
                        >
                        ...
                        bookings.slice(0, 5).map((booking) => {
                            ...
                            return (
                                <div key={booking.id} className="flex items-center justify-between px-6 py-4 hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <span className={`text-xs font-bold px-3 py-1 rounded-none border shrink-0 ${statusColors[booking.status] || statusColors.Pending}`}>
                                        {booking.status}
                                    </span>
                                </div>
                            );
